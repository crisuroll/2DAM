...
public class BST {
    ...
    public int maxValueRecursive(Node current) {
        int max,maxL,maxR;
        ...
        max = current.value;
        maxL = maxValueRecursive(current.left);
        maxR = maxValueRecursive(current.right);

        return Math.max(Math.max(maxL, maxR), max) ;
    }
	 ...
	 * ÁRBOLES IDÉNTICOS
	 */
	public boolean equal(BST other) {
		return equalRecursive(this.root, other.root);
	}
    ...
    public int countLeavesRecursive(Node current) {
        if (current == null) {
            return 0;
        }
